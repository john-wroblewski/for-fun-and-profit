 ...
 *      Author: john wroblewski (email) wroblewski2 [at] wisc [dot] edu
 */
...
/* postcondition: returned queue contains all the tokens, and pqueue_infix should be
   empty */
struct token_queue infix_to_postfix(struct token_queue * pqueue_infix) {
	struct token_queue queue_postfix;			/* create new queue for postfix notation.. */
	queue_postfix.front = queue_postfix.back = NULL;	/* and initialize front and back to NULL */

	p_expr_token p_op_stack = NULL;	/* pointer to top of operator stack */
	...
	while( ( ptoken = dequeue( pqueue_infix ) ) ) {
		if( ptoken->type == OPERAND )
			enqueue( &queue_postfix , ptoken );
				...
				/* while tokens are on stack */
				while( p_op_stack && p_op_stack->type == OPERATOR &&
				     ( ( op_associativity[ op_precedences[ ptoken->value.op_code ] ] == LEFT && op_precedences[ ptoken->value.op_code ] ==  op_precedences[ p_op_stack->value.op_code ] )
				       || op_precedences[ ptoken->value.op_code ] < op_precedences[ p_op_stack->value.op_code ] ) )
					/* pop operator off stack and onto output queue */
					enqueue( &queue_postfix , pop( &p_op_stack ) );
			/* push operator from input queue onto stack */
			push( &p_op_stack , ptoken );
		}
#if PARSE_PARENS
			...
			while( p_op_stack->type != LPARENS )
				enqueue( &queue_postfix , pop( &p_op_stack ) );
			free( ptoken );
			free( ( ptoken = pop( &p_op_stack ) ) );
		}
#endif
	}
	/* pop rest of tokens off stack onto output_queue */
	while( p_op_stack )
		enqueue( &queue_postfix , pop( &p_op_stack ) );
...
#define OPERAND_TOKEN_CHECK  (pstack && pstack->type == OPERAND)

double failure( p_expr_token pstack , struct token_queue * pqueue_postfix ) {
        p_expr_token ptoken;
        /* free tokens on stack */
        for( ptoken = pop( &pstack ); ptoken; ptoken = pop( &pstack ) )
                free( ptoken );
        /* free tokens in queue */
        for( ptoken = dequeue( pqueue_postfix ); ptoken; ptoken = dequeue( pqueue_postfix ) )
                free( ptoken );
	return 0;
}
	union token_value value;
	double operands[ 2 ];
	unsigned int i;
	p_expr_token pstack = NULL , ptoken;

	while( ( ptoken = dequeue( pqueue_postfix ) ) ) {
		if( ptoken->type == OPERATOR ) {
			value.op_code = ptoken->value.op_code;
			free( ptoken );
			for( i = 0; i < op_operands[ value.op_code ]; i++ )
				if( OPERAND_TOKEN_CHECK ) {
					operands[ i ] = ( ptoken = pop( &pstack ) )->value.operand;
					free( ptoken );
				}
				else {
					puts( "error: insufficient operands" );
					return failure( pstack , pqueue_postfix );
					break;
				}
			...
			case DIVIDE:
				...
				break;
			case NEGATE:
				value.operand = operands[ 0 ] * ( -1 );
				break;
			}
			push( &pstack , new_token( OPERAND , value ) );
		}
		/* not operator, must be operand */
		else
			push( &pstack , ptoken );
	}
	value.operand = pstack->value.operand;
	free( pstack );
